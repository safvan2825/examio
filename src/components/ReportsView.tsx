...
export const ReportsView: React.FC<Props> = ({ records, students, classes, subjects }) => {
  ...
  const filtered = useMemo(() => records.filter(r => {
    const studentMatch = !studentId || r.studentId === studentId;
    const classMatch = !classId || r.classId === classId;
    ...
    if (type === 'student') return studentMatch;
    if (type === 'class') return classMatch;
    if (type === 'subject') return classMatch && subjectMatch;
  ...
  const exportExcel = () => {
    ...
    const ws = XLSX.utils.json_to_sheet(data); ws['!freeze']={xSplit:0,ySplit:1};
    ws['!cols']=[{wch:8},{wch:18},{wch:30},{wch:16},{wch:22},{wch:14}];
    const wb=XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb,ws,'Absentee Report');
    XLSX.writeFile(wb,`Absentee_Report_${new Date().toISOString().slice(0,10)}.xlsx`);
  };
  const exportPdf = () => {
    const doc=new jsPDF({orientation:'landscape',unit:'mm',format:'a4'});
    doc.setFont('helvetica','bold'); doc.setFontSize(15); doc.text('Noorul Huda Examination Board',148.5,12,{align:'center'});
    doc.setFontSize(11); doc.text(title,148.5,19,{align:'center'});
    doc.setFont('helvetica','normal'); doc.setFontSize(8); doc.text(`Total Records: ${filtered.length} | Generated: ${new Date().toLocaleString()}`,148.5,25,{align:'center'});
  ...
  return <div className="space-y-5">
    ...
    <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3">
        ...
        {(type!=='student'||type==='custom') && <select value={classId} onChange={e=>setClassId(e.target.value)} className="border rounded-lg px-3 py-2.5 text-xs"><option value="">All Classes</option>{classes.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select>}
