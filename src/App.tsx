...
export default function App(){
 ...
 if(exam)return <div className="examio-workspace-shell"><ExaminationWorkspace uid={user.uid} campus={campus} exam={exam} categories={categories} classes={classes} subjects={subjects} students={students} assignments={assignments} rooms={examRooms} arrangements={arrangements} onSaveRoom={(room)=>dbApi.saveExamRoom(exam.id,room)} onDeleteRoom={(id)=>dbApi.deleteExamRoom(exam.id,id)} onDeleteBulkRooms={(ids)=>dbApi.deleteBulkExamRooms(exam.id,ids)} onSaveArrangement={dbApi.saveSeatingArrangement} onOpenPrintModal={(type,session,arrangement)=>setPrintRequest({type,session,arrangement})} onBack={()=>{setExam(null);setExamRooms([])}}/>{printRequest&&<PrintModalView type={printRequest.type} session={printRequest.session} arrangement={printRequest.arrangement} categories={categories} classes={classes} onClose={()=>setPrintRequest(null)}/>}</div>;
