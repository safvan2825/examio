...
import {
  subscribeCategories, subscribeClasses, subscribeStudents, subscribeRooms, subscribeSessions,
  subscribeSeatingArrangements, subscribeAdminCredentials, DEFAULT_CREDENTIALS,
  saveCategory, deleteCategory, deleteBulkCategories, saveClassItem, saveBulkClasses,
  deleteClassItem, deleteBulkClasses, saveStudent, saveBulkStudents, deleteStudent,
  deleteBulkStudents, saveRoom, deleteRoom, deleteBulkRooms, saveSession, deleteSession,
  saveSeatingArrangement, seedSampleData, clearAllData,
} from './lib/realtime';
...
const normalizeArrangement = (arr: SeatingArrangement): SeatingArrangement => ({
  ...arr,
  manualAllocations: toArray(arr.manualAllocations), onlineAllocations: toArray(arr.onlineAllocations),
  roomDiagrams: toArray<any>(arr.roomDiagrams).map(diagram => ({...diagram, sides: toArray<any>(diagram.sides).map(side => ({...side, grid: toArray<any>(side.grid).map(row => toArray<any>(row))})), classSummary: toArray<any>(diagram.classSummary)})),
...
export default function App() {
  ...
  useEffect(() => {
    const unsubCat=subscribeCategories(setCategories), unsubCls=subscribeClasses(setClasses), unsubSt=subscribeStudents(setStudents);
    const unsubRm=subscribeRooms(data=>setRooms(data.map(normalizeRoom))), unsubSess=subscribeSessions(data=>setSessions(data.map(normalizeSession)));
    const unsubSeat=subscribeSeatingArrangements(data=>setSeatingArrangements(data.map(normalizeArrangement)));
    const unsubSubjects=subscribeSubjects(setSubjects), unsubAbs=subscribeAbsenteeRecords(setAbsenteeRecords);
    return ()=>{unsubCat();unsubCls();unsubSt();unsubRm();unsubSess();unsubSeat();unsubSubjects();unsubAbs();};
  }, []);
  ...
  const handleLoginSuccess=()=>{localStorage.setItem('nh_authenticated','true');setIsAuthenticated(true)};
  const handleLogout=()=>{localStorage.removeItem('nh_authenticated');setIsAuthenticated(false)};
  const handleSelectSessionForGenerator=(sessionId:string)=>{setSelectedGeneratorSessionId(sessionId);setActiveTab('generator')};
  const handleOpenPrintModal=(type:'roomDiagram'|'studentList',session:ExamSession,arrangement:SeatingArrangement)=>setPrintModalState({isOpen:true,type,session,arrangement});

  if(!isAuthenticated) return <LoginView adminCredentials={adminCredentials} onLoginSuccess={handleLoginSuccess}/>;

  return <div className="min-h-screen bg-slate-50 text-slate-800 font-sans flex selection:bg-blue-600 selection:text-white">
    <Sidebar activeTab={activeTab} onTabChange={setActiveTab} onLogout={handleLogout}/>
    <main className="flex-1 p-6 overflow-y-auto bg-slate-50 space-y-6">
      {activeTab==='dashboard'&&<DashboardView categories={categories} classes={classes} students={students} rooms={rooms} sessions={sessions} onNavigate={setActiveTab} onSelectSessionForGenerator={handleSelectSessionForGenerator}/>} 
    ...
    {printModalState.isOpen&&printModalState.session&&printModalState.arrangement&&<PrintModalView type={printModalState.type} session={printModalState.session} arrangement={printModalState.arrangement} categories={categories} classes={classes} onClose={()=>setPrintModalState({...printModalState,isOpen:false})}/>} 
    {!isOnline&&<div className="fixed bottom-4 right-4 bg-amber-100 text-amber-800 border border-amber-200 px-3 py-2 rounded-lg text-xs font-semibold shadow-lg">Offline — Firebase changes may fail until connection returns.</div>}
  </div>;
}
