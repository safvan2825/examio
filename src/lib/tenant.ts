...
export const subscribeTimetable = (uid: string, campusId: string, examId: string, callback: (items: ExamTimetableEntry[]) => void) => onValue(ref(db, timetablePath(uid,campusId,examId)), snap => { const value = snap.val() || {}; callback(Object.values(value) as ExamTimetableEntry[]); }, error => { console.error('Timetable load failed:', error); callback([]); });
export const saveTimetableEntry = async (uid: string, campusId: string, entry: ExamTimetableEntry) => { await set(ref(db, `${timetablePath(uid,campusId,entry.examinationId)}/${entry.id}`), entry); };
export const deleteTimetableEntry = async (uid: string, campusId: string, examId: string, entryId: string) => { await remove(ref(db, `${timetablePath(uid,campusId,examId)}/${entryId}`)); };
